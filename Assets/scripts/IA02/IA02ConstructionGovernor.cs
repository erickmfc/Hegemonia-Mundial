...
{
    public sealed class IA02ConstructionGovernor
    {
        ...
        private IA02ConstructionMode mode = IA02ConstructionMode.Active;
        ...
        public IA02ConstructionGovernor(IA02Controller controller, IA02RuntimeContext context, IA02NationProfile profile)
        {
        ...
        public IA02ConstructionMode ConstructionMode => mode;
        public string ConstructionFreezeReason => freezeReason;
        public string NextUnfreezeCondition => nextUnfreezeCondition;
        public string CurrentConstructionState { get; private set; } = IA02ConstructionState.Idle.ToString();
        ...
        public void Refresh(float now, DadosPaisGoverno country, IA02WorldState world, IA02BuildCatalogAdapter catalog, IA02BuildDirector buildDirector)
        {
            ...
            IA02ConstructionGovernorSettings settings = profile != null ? profile.ConstructionGovernor : null;
            IA02ConstructionPhaseLimit phaseLimit = settings != null ? settings.ResolvePhaseLimit(context.CurrentStage) : null;
            ...
            bool threatened = IA02OperationalRules.IsCapitalThreatened(world, controller != null && buildDirector != null ? buildDirector.CapitalMarker : null, country);
            ...
            FixedDefenseCount = manager != null && manager.WorldRegistry != null
                ? manager.WorldRegistry.CountStructuresByStrategicRole(context.TeamId, IA02StrategicRole.FixedDefense)
                    + manager.WorldRegistry.CountStructuresByStrategicRole(context.TeamId, IA02StrategicRole.AntiAirDefense)
                    + manager.WorldRegistry.CountStructuresByStrategicRole(context.TeamId, IA02StrategicRole.CoastalDefense)
                : 0;
            ...
            bool economyCanExpand = treasury >= Mathf.Max(14000, (settings != null ? settings.EmergencyReserve : 0) * 2)
                && food > 0 && energy > 0;
            if (!frozen && !openingInfrastructurePending && context.CurrentPosture == IA02NationPosture.Recovery && !threatened && !atWar && !economyCanExpand)
            {
                frozen = true;
        ...
        {
            return mode == IA02ConstructionMode.Frozen
                && !string.IsNullOrWhiteSpace(commandId)
                && commandId.StartsWith("build:", System.StringComparison.OrdinalIgnoreCase);
        }

        public void SetConstructionState(IA02ConstructionState state, string commandId = null)
        {
            CurrentConstructionState = state.ToString();
            ActiveConstructionCommand = string.IsNullOrWhiteSpace(commandId) ? "n/d" : commandId;
        }

        private void SyncConstructionState(IA02BuildDirector buildDirector)
        {
            if (buildDirector == null)
            {
                CurrentConstructionState = IA02ConstructionState.Idle.ToString();
        ...
        private string ResolveCoveragePercent(int structureCount, IA02ConstructionPhaseLimit phaseLimit)
        {
        ...
        {
            IA02Manager manager = controller != null ? controller.Manager : null;
            IA02WorldRegistry registry = manager != null ? manager.WorldRegistry : null;
            if (registry == null || context == null)
            {
                return false;
            }

            bool hasTent = HasStructureMatching(registry, IA02StrategicRole.MilitaryProduction, "tenda", "tent");
            bool hasVehicleConstructor = HasStructureMatching(registry, IA02StrategicRole.MilitaryProduction, "construtor", "veiculo", "veículo", "vehicle");
            bool hasMilitaryAirport = HasStructureMatching(registry, IA02StrategicRole.Airfield, "militar", "military", "aeroporto_militar");
            bool hasCommercialAirport = HasStructureMatching(registry, IA02StrategicRole.Airfield, "comercial", "commercial", "aeroporto_comercial");
            bool hasShipyard = registry.CountStructuresByStrategicRole(context.TeamId, IA02StrategicRole.NavalBase) > 0
                || registry.CountStructuresByStrategicRole(context.TeamId, IA02StrategicRole.Shipyard) > 0
                || registry.CountStructuresByStrategicRole(context.TeamId, IA02StrategicRole.Port) > 0
                || registry.CountStructuresByStrategicRole(context.TeamId, IA02StrategicRole.Pier) > 0;

            return !hasTent || !hasVehicleConstructor || !hasMilitaryAirport || !hasCommercialAirport || !hasShipyard;
        }

        private bool HasStructureMatching(IA02WorldRegistry registry, IA02StrategicRole role, params string[] tokens)
        {
        ...
        private string BuildRoleSummary(IA02WorldState world)
        {
            roleSummary.Clear();
            if (world == null)
            {
            ...
            int unknown = 0;
            IA02Manager manager = controller != null ? controller.Manager : null;
            IReadOnlyList<IA02WorldEntityRecord> records = manager != null && manager.WorldRegistry != null
            ...
            for (int i = 0; i < records.Count; i++)
            {
                ...
                IA02StrategicRole role = record.StrategicRole;
                if (role == IA02StrategicRole.None && !string.IsNullOrWhiteSpace(record.Category))
                {
                    switch (record.Category.Trim())
                    {
                        case "Command":
                            role = IA02StrategicRole.Command;
                            break;
                        case "Residential":
                            role = IA02StrategicRole.Residential;
                            break;
                        case "Agricultural":
                            role = IA02StrategicRole.FoodProduction;
                            break;
                        case "Industrial":
                            role = IA02StrategicRole.Industrial;
                            break;
                        case "Energy":
                            role = IA02StrategicRole.EnergyProduction;
                            break;
                        case "Storage":
                            role = IA02StrategicRole.Storage;
                            break;
                        case "Logistics":
                            role = IA02StrategicRole.Logistics;
                            break;
                        case "Military":
                            role = IA02StrategicRole.MilitaryProduction;
                            break;
                        case "Defense":
                            role = IA02StrategicRole.FixedDefense;
                            break;
                        case "Air":
                            role = IA02StrategicRole.Airfield;
                            break;
                        case "Naval":
                            role = IA02StrategicRole.NavalBase;
                            break;
                        case "Research":
                            role = IA02StrategicRole.Research;
                            break;
                    }
                }
