...
{
    /// <summary>
    /// Garante a reserva militar mínima da IA02. A construção civil continua sendo
    ...
    /// </summary>
    public sealed class IA02MilitaryDirector
    {
        ...
        private readonly IA02BuildDirector buildDirector;
        ...
        private readonly List<IA02WarAdvanceZone> airWarZonesBuffer = new List<IA02WarAdvanceZone>(8);
        ...
        private readonly List<IA02NavalPatrolZone> navalPatrolZonesBuffer = new List<IA02NavalPatrolZone>(8);
        private readonly List<IA02WarAdvanceZone> navalWarZonesBuffer = new List<IA02WarAdvanceZone>(8);
        ...
        public IA02MilitaryDirector(IA02Controller controller, IA02RuntimeContext context, IA02BuildDirector buildDirector)
        {
        ...
        public bool Tick(float now)
        {
            ...
            {
                string orderId;
                bool reserved = IA02MilitaryProductionGuard.TryReserve(context.TeamId, IA02MilitaryAssetKind.Infantry, targetSoldiers, soldiers, now, 45f, out orderId);
                bool produced = reserved && TryProduceLand(FindSoldier(), "soldados", orderId);
                if (reserved && !produced) IA02MilitaryProductionGuard.Cancel(context.TeamId, IA02MilitaryAssetKind.Infantry, now);
                changed |= produced;
                if (produced) actions++;
            }
            ...
            {
                string orderId;
                bool reserved = IA02MilitaryProductionGuard.TryReserve(context.TeamId, IA02MilitaryAssetKind.Tank, targetTanks, tanks, now, 45f, out orderId);
                bool produced = reserved && TryProduceLand(FindTank(), "tanques", orderId);
                if (reserved && !produced) IA02MilitaryProductionGuard.Cancel(context.TeamId, IA02MilitaryAssetKind.Tank, now);
                changed |= produced;
                if (produced) actions++;
            }
            ...
            // Toda IA02 procura manter pelo menos uma defesa antiaerea Ares_Ar
            // quando existe uma ficha valida no catalogo e uma fabrica propria.
            if (productionAllowed && antiAir < 1 && actions < 2)
            {
                string orderId;
                bool reserved = IA02MilitaryProductionGuard.TryReserveSingle(context.TeamId, IA02MilitaryAssetKind.AntiAir, antiAir, now, 45f, out orderId);
                bool produced = reserved && TryProduceLand(FindAntiAir(), "Ares_Ar antiaereo", orderId);
                if (reserved && !produced) IA02MilitaryProductionGuard.Cancel(context.TeamId, IA02MilitaryAssetKind.AntiAir, now);
                changed |= produced;
                if (produced) actions++;
            }
            ...
            {
                string orderId;
                bool reserved = IA02MilitaryProductionGuard.TryReserve(context.TeamId, IA02MilitaryAssetKind.Fighter, targetFighters, fighters, now, 120f, out orderId);
                bool produced = reserved && TryProduceFighter(FindFighter(), now, orderId);
                if (reserved && !produced) IA02MilitaryProductionGuard.Cancel(context.TeamId, IA02MilitaryAssetKind.Fighter, now);
                changed |= produced;
                if (produced) actions++;
            }
            ...
            {
                string orderId;
                bool reserved = IA02MilitaryProductionGuard.TryReserve(context.TeamId, IA02MilitaryAssetKind.Naval, targetNaval, naval, now, 180f, out orderId);
                bool produced = reserved && TryProduceNaval(FindNaval(), "navios", orderId);
                if (reserved && !produced) IA02MilitaryProductionGuard.Cancel(context.TeamId, IA02MilitaryAssetKind.Naval, now);
                changed |= produced;
                if (produced) actions++;
            }
            ...
            IAProductionDiagnostics fighterDiagnostics = IAAutoProductionRegistry.GetDiagnostics(
                context.TeamId,
                IA02MilitaryAssetKind.Fighter.ToString(),
                targetFighters,
                fighters);
            ...
            if (changed)
            {
                Debug.Log("[IA02 Military] " + status);
            }
            return changed;
        }
        ...
        private void EnsureOperationalWarZones()
        {
            ...
            if (shipyard != null)
            {
                CriarZonaGuerraSeAusente(shipyard.transform, "IA02 WarAdvanceZone Naval A", new Vector3(180f, 0f, 220f), IA02WarAdvanceZone.Dominio.Naval, 180f);
                CriarZonaGuerraSeAusente(shipyard.transform, "IA02 WarAdvanceZone Naval B", new Vector3(-220f, 0f, 300f), IA02WarAdvanceZone.Dominio.Naval, 180f);
                CriarZonaExtracaoSeAusente(shipyard.transform, "IA02 ExtractionZone Naval", new Vector3(-40f, 0f, 150f));
            }
            ...
            for (int i = 0; i < airports.Length; i++)
            {
                ...
                CriarZonaGuerraSeAusente(airport.transform, "IA02 WarAdvanceZone Aerea", new Vector3(0f, 100f, 320f), IA02WarAdvanceZone.Dominio.Aereo, 260f);
                airportFound = true;
                break;
            }
        ...
        private void CriarZonaGuerraSeAusente(Transform parent, string nome, Vector3 local, IA02WarAdvanceZone.Dominio dominio, float raio)
        {
            ...
            IA02WarAdvanceZone zona = child.GetComponent<IA02WarAdvanceZone>();
            if (zona == null) zona = child.gameObject.AddComponent<IA02WarAdvanceZone>();
        ...
        private void CriarZonaExtracaoSeAusente(Transform parent, string nome, Vector3 local)
        {
            ...
            IA02ExtractionZone zona = child.GetComponent<IA02ExtractionZone>();
            if (zona == null) zona = child.gameObject.AddComponent<IA02ExtractionZone>();
        ...
        private void ApplyNavalPatrols(float now)
        {
            ...
            IA02NavalPatrolZone[] zones = controller != null
                ? controller.GetComponentsInChildren<IA02NavalPatrolZone>(true)
                : Array.Empty<IA02NavalPatrolZone>();
            ...
            if (zones.Length == 0)
            {
                zones = UnityEngine.Object.FindObjectsByType<IA02NavalPatrolZone>(
            ...
            for (int i = 0; i < registeredIdentitiesBuffer.Count; i++)
            {
                ...
                IA02NavalPatrolZone zone = zones[sector];
                if (!zone.TryCriarRota(assignedBySector[sector], out Vector3[] route))
                {
                    rejected++;
                    if (EmitirLogsDetalhadosDePatrulha)
                        Debug.Log("[IA02 Military] Rota naval ignorada por conter ponto fora da água: " + zone.name);
                    continue;
                }
                if (control.EmitirOrdemPatrulha(route))
                {
                    ...
                    assigned++;
                    DiagnosticoDesempenhoJogo.RegistrarEvento("IA02_NavalPatrol", id.name + " patrulha " + zone.name);
                }
                else
                {
                    rejected++;
                    Debug.LogWarning("[IA02 Military] Patrulha recusada: " + id.name + " ordem=" + control.OrdemAtual);
                }
            }
            if (candidates > 0)
            {
            ...
            else
            {
                ...
                if (navalIdentities > 0 && now >= nextUnlinkedNavalWarningAt)
                {
                    ...
                    Debug.LogWarning(string.Format("[IA02 Military] Navios encontrados sem vínculo com o time {0}: identidades navais={1}",
        ...
        private void ApplyNavalIncidentResponses(float now)
        {
            ...
            IA02WarAdvanceZone[] zones = UnityEngine.Object.FindObjectsByType<IA02WarAdvanceZone>(FindObjectsInactive.Include, FindObjectsSortMode.None);
            for (int i = 0; i < zones.Length; i++)
            {
                IA02WarAdvanceZone zone = zones[i];
                if (zone != null && zone.TeamId == context.TeamId && zone.Tipo == IA02WarAdvanceZone.Dominio.Naval
            ...
            navalPatrolZonesBuffer.AddRange(UnityEngine.Object.FindObjectsByType<IA02NavalPatrolZone>(FindObjectsInactive.Include, FindObjectsSortMode.None));
            IA02WorldState world = controller != null && controller.Runtime != null ? controller.Runtime.WorldState : null;

            for (int z = 0; z < navalWarZonesBuffer.Count; z++)
            {
                IA02WarAdvanceZone zone = navalWarZonesBuffer[z];
                ...
                for (int incidentIndex = 0; incidentIndex < zone.IncidentesAtivos.Count; incidentIndex++)
                {
                    ...
                    for (int c = 0; c < navalResponseCandidatesBuffer.Count && issued < responseLimit; c++)
                    {
                        ...
                        IA02NavalPatrolZone patrolZone = FindNearestNavalPatrolZone(target);
                        ...
                        issued++;
                        DiagnosticoDesempenhoJogo.RegistrarEvento("IA02_NavalIncidentResponse", candidate.Identity.name + " -> " + incident.MissionState + " em " + target);
        ...
        private void AtualizarContatosNavais(IA02WarAdvanceZone zone, IA02WorldState world, float now)
        {
        ...
        private void BuildNavalResponseCandidates(WarAdvanceIncident incident, IA02WarAdvanceZone zone)
        {
        ...
        private IA02NavalPatrolZone FindNearestNavalPatrolZone(Vector3 target)
        {
            IA02NavalPatrolZone nearest = null;
            float best = float.MaxValue;
            for (int i = 0; i < navalPatrolZonesBuffer.Count; i++)
            {
                IA02NavalPatrolZone zone = navalPatrolZonesBuffer[i];
        ...
        private static int EscolherSetorNaval(IList<IA02NavalPatrolZone> zones, int[] assignedBySector)
        {
            int selected = 0;
        ...
        private void ApplyNavalCombat(float now)
        {
            ...
            RegistroEntidadesJogo.FillUnidades(navalUnitsBuffer);
            IA02WorldState world = controller != null && controller.Runtime != null
            ...
            for (int i = 0; i < navalUnitsBuffer.Count && engajados < limiteEngajamentos; i++)
            {
                ...
                engajados++;
                DiagnosticoDesempenhoJogo.RegistrarEvento("IA02_NavalCombat", identidade.name + " -> " + alvo.name);
            }

            DiagnosticoDesempenhoJogo.DefinirContadorMetrica("ia02_naval_engaged", engajados);
            DiagnosticoDesempenhoJogo.DefinirContadorMetrica("ia02_naval_launchers_active", lancadoresAtivos);
        }
        ...
        private Transform EncontrarInimigoNavalMaisProximo(Transform origem, IA02WorldState world)
        {
        ...
        private void ApplyAirPatrols(float now)
        {
            ...
            for (int i = 0; i < airUnitsBuffer.Count; i++)
            {
                ...
                if (control.EmitirOrdemPatrulha(route))
                {
                    assignedBySector[sector]++;
                    assigned++;
                    DiagnosticoDesempenhoJogo.RegistrarEvento("IA02_AirPatrol", id.name + " patrulha nos 4 Creates militares");
                    if (EmitirLogsDetalhadosDePatrulha)
                    {
                        Debug.Log("[IA02 Military] Patrulha aerea: " + id.name + " -> ciclo Create 01/02/03/04");
                    }
                }
            }
            if (candidates > 0)
            {
                DiagnosticoDesempenhoJogo.DefinirContadorMetrica("ia02_air_patrol_assigned", assigned);
                if (EmitirLogsDetalhadosDePatrulha)
                {
                    Debug.Log("[IA02 Military] Patrulha aerea: candidatos=" + candidates + " | atribuídos=" + assigned);
                }
            }
        }
        ...
        private void ApplyAirIncidentResponses(float now)
        {
            ...
            {
                IA02WarAdvanceZone zone = zones[i];
                if (zone != null && zone.TeamId == context.TeamId && zone.Tipo == IA02WarAdvanceZone.Dominio.Aereo && zone.ResponderAtaques)
            ...
            {
                IA02WarAdvanceZone zone = airWarZonesBuffer[z];
                zone.LimparIncidentesExpirados(now);
                for (int incidentIndex = 0; incidentIndex < zone.IncidentesAtivos.Count; incidentIndex++)
                {
                    WarAdvanceIncident incident = zone.IncidentesAtivos[incidentIndex];
                    ...
                    if (responseLimit <= 0) continue;
                    BuildAirResponseCandidates(incident, zone, now);
                    int issued = 0;
                    for (int c = 0; c < airResponseCandidatesBuffer.Count && issued < responseLimit; c++)
                    {
                        ...
                        IA02AirPatrolZone patrolZone = FindNearestAirPatrolCreate(candidate.Aircraft, incident.EnemyLastKnownPosition != Vector3.zero ? incident.EnemyLastKnownPosition : incident.Position);
                        ...
                        {
                            // Interceptar ainda não significa abrir fogo: o
                            // avião vai à última posição conhecida em modo de
                            // deslocamento, deixando a confirmação/combate
                            // para o próximo ciclo de contato.
                        ...
                        if (zone.RetornarPatrulhaDepois)
                            airResponseAssignmentUntil[key] = incident.ExpirationTime;
                        issued++;
                        DiagnosticoDesempenhoJogo.RegistrarEvento("IA02_AirIncidentResponse", candidate.Identity.name + " -> " + incident.MissionState + " em " + target);
                    }
                }
            }
        }

        private void BuildAirResponseCandidates(WarAdvanceIncident incident, IA02WarAdvanceZone zone, float now)
        {
        ...
        private IA02AirPatrolZone FindNearestAirPatrolCreate(ControleAviao aircraft, Vector3 target)
        {
            IA02AirPatrolZone nearest = null;
            float best = float.MaxValue;
            for (int i = 0; i < airPatrolCreatesBuffer.Count; i++)
            {
                IA02AirPatrolZone create = airPatrolCreatesBuffer[i];
        ...
        private bool TryObterCreatesPatrulhaAerea(List<IA02AirPatrolZone> destino)
        {
            ...
            for (int i = 0; i < airports.Length; i++)
            {
                ...
                Transform grupo = airport.transform.Find("IA02 Creates Patrulha Aerea");
                if (grupo == null) continue;
                for (int ponto = 1; ponto <= 4; ponto++)
                {
                    Transform create = grupo.Find("Create Patrulha Aerea " + ponto.ToString("00"));
                    IA02AirPatrolZone zone = create != null ? create.GetComponent<IA02AirPatrolZone>() : null;
            ...
            // (IA02AirportBuildSlot), antes de o prefab do GerenciadorAeroporto
            ...
            IA02AirportBuildSlot[] airportSlots = UnityEngine.Object.FindObjectsByType<IA02AirportBuildSlot>(FindObjectsInactive.Include, FindObjectsSortMode.None);
            for (int i = 0; i < airportSlots.Length; i++)
            {
                IA02AirportBuildSlot airport = airportSlots[i];
                IA02BuildSlot slot = airport != null ? airport.GetComponent<IA02BuildSlot>() : null;
        ...
        private int EnsureAirPatrolCreates()
        {
            ...
            for (int i = 0; i < airports.Length; i++)
            {
                ...
                if (grupo == null)
                {
                    grupo = new GameObject("IA02 Creates Patrulha Aerea").transform;
                ...
                for (int ponto = 1; ponto <= 4; ponto++)
                {
                    ...
                    IA02AirPatrolZone zone = create.GetComponent<IA02AirPatrolZone>();
                    if (zone == null) zone = create.gameObject.AddComponent<IA02AirPatrolZone>();
                }
            ...
            for (int i = 0; i < airportSlots.Length; i++)
            {
                ...
                for (int ponto = 1; ponto <= 4; ponto++)
                {
                    ...
                    IA02AirPatrolZone zone = create.GetComponent<IA02AirPatrolZone>();
                    if (zone == null) create.gameObject.AddComponent<IA02AirPatrolZone>();
                }
        ...
        private static int EscolherSetorPatrulha(IList<IA02AirPatrolZone> creates, int[] alocados, int fallback)
        {
        ...
        private static Vector3[] CriarRotaDosCreates(IList<IA02AirPatrolZone> creates, int setor, int indiceRota)
        {
        ...
        private static bool EhAeroportoMilitar(IA02AirportBuildSlot airport, IA02BuildSlot slot)
        {
        ...
        private void ApplyNavalStaging(float now)
        {
            ...
            if (zones.Length == 0) return;
            IA02WarAdvanceZone objectiveZone = null;
            if (controller != null && controller.WarEscalationLevel > 0)
            {
                IA02WarAdvanceZone[] objectives = UnityEngine.Object.FindObjectsByType<IA02WarAdvanceZone>(FindObjectsInactive.Include, FindObjectsSortMode.None);
                for (int o = 0; o < objectives.Length; o++)
                {
                    IA02WarAdvanceZone candidate = objectives[o];
                    if (candidate != null && candidate.TeamId == context.TeamId && candidate.Tipo == IA02WarAdvanceZone.Dominio.Naval
            ...
            for (int i = 0; i < registeredIdentitiesBuffer.Count; i++)
            {
                ...
                Vector3 direcao = ponto - id.transform.position;
                if (direcao.sqrMagnitude < 0.01f) direcao = Vector3.forward;
                if (!NavalPlacementResolver.TryResolveWaterSpawn(ponto, direcao, 0f, 300f, out Vector3 agua, out _, out string reason))
                {
                    // Nunca envie um navio para o fallback do marcador quando
                    // ele estiver em terra. A ordem fica pendente para o
                    // próximo ciclo, sem gerar destino naval inválido.
                    if (EmitirLogsDetalhadosDePatrulha)
                        Debug.Log("[IA02 Military] Ponto naval ignorado: " + zones[(i + 1) % zones.Length].name + " -> " + reason);
                    continue;
                }
                ponto = agua;
                if (control.EmitirOrdemMover(ponto, true))
                {
                    ...
                    if (EmitirLogsDetalhadosDePatrulha)
                    {
                        Debug.Log("[IA02 Military] Navio em ponto costeiro otimizado: " + id.name + " -> " + ponto.ToString("F2"));
                    }
                    if (staged >= 2) break;
                }
            }
        ...
        private void EnsurePierThenPlatform(float now)
        {
            ...
            {
                if (buildDirector != null && buildDirector.IsRebuildBlocked(IA02IntentType.BuildPier, now))
                {
                ...
                BuildCoastalStructure(IA02IntentType.BuildPier, "pier", "dock");
                return;
            }
            if (!ownPier || now < nextPlatformRecoveryAt) return;

            if (buildDirector != null && buildDirector.IsRebuildBlocked(IA02IntentType.BuildOffshorePlatform, now))
            {
            ...
            BuildCoastalStructure(IA02IntentType.BuildOffshorePlatform, "plataforma", "offshore");
        }
        ...
        private void EnsureTankerAfterPlatform(float now)
        {
            ...
            if (platformConfirmedAt < 0f)
            {
                ...
                Debug.Log("[IA02 Military] Plataforma de petróleo confirmada; petroleiro liberado em "
            ...
            DiagnosticoDesempenhoJogo.DefinirContadorMetrica("ia02_tankers_active", petroleirosAtivos);
            ...
            if (fichaPetroleiro == null || !fichaPetroleiro.TryGetPrefabBasico(out GameObject prefab) || prefab == null)
            {
                ...
                Debug.LogWarning("[IA02 Military] Plataforma confirmada, mas ficha de navio petroleiro ainda nao foi localizada.");
                return;
            }

            Estaleiro[] estaleiros = UnityEngine.Object.FindObjectsByType<Estaleiro>(
                FindObjectsInactive.Include, FindObjectsSortMode.None);
            if (!IA02MilitaryProductionGuard.TryReserveSingle(
                context.TeamId,
                IA02MilitaryAssetKind.OilTanker,
                petroleirosAtivos,
                now,
                90f,
            ...
            for (int i = 0; i < estaleiros.Length; i++)
            {
                ...
                IA02MilitaryProductionGuard.ConfirmQueued(tankerOrderId, estaleiro.GetInstanceID(), now);

                tankerOrdersIssued++;
                lastTankerOrderDay = diaAtual;
                Debug.Log("[IA02 Military] Navio petroleiro iniciado em cadencia segura: "
                    + estaleiro.name + " -> " + prefab.name);
                return;
            }

            IA02MilitaryProductionGuard.Cancel(context.TeamId, IA02MilitaryAssetKind.OilTanker, now);
            nextTankerAttemptAt = now + 15f;
        }

        private bool BuildCoastalStructure(IA02IntentType intent, params string[] blueprintTokens)
        {
            ...
            IA02BuildExecutor.NormalizeStructureIdentity(built, context.TeamId, controller.NationName);
            ...
            Debug.Log("[IA02 Military] " + intent + " criado no create: " + built.name + " pos=" + built.transform.position.ToString("F2"));
            return true;
        }
        ...
        private void AddCandidate(DadosConstrucao item, IReadOnlyList<DadosConstrucao> configuredMilitary)
        {
            ...
            if (!IA02MilitaryCatalogPolicy.IsAllowed(item, configuredMilitary)) return;
            try
            {
                GameObject prefab;
        ...
        private bool TryProduceLand(DadosConstrucao item, string label, string orderId = "")
        {
            ...
            for (int i = 0; i < registeredFactoriesBuffer.Count; i++)
            {
                ...
                if (produced != null)
                {
                    ...
                    IA02MilitaryProductionGuard.Complete(orderId, Time.time);
                    return true;
                }
        ...
        private bool TryProduceFighter(DadosConstrucao item, float now, string orderId = "")
        {
            ...
            for (int i = 0; i < airports.Length; i++)
            {
                ...
                Debug.Log("[IA02 Military] Caca estacionado no aeroporto proprio: " + airport.name + " -> " + aircraft.name);
                return true;
        ...
        private GerenciadorAeroporto EnsureOwnMilitaryAirport()
        {
            ...
            Vector3 anchor;
            Quaternion rotation;
            if (!controller.TryResolveConstructionAnchor(IA02IntentType.BuildMilitaryAirport, out anchor, out rotation))
            {
                Debug.LogWarning("[IA02 Military] Create de aeroporto militar nao resolvido para a IA02.");
                return null;
            }
            if (!controller.IsPositionInsidePreparedTerritory(anchor, 220f))
            {
                Debug.LogWarning("[IA02 Military] Anchor de aeroporto fora do territorio preparado; criacao cancelada.");
                return null;
            }
            ...
            {
                Debug.LogWarning("[IA02 Military] Ficha/prefab do aeroporto militar nao encontrado.");
                return null;
            }
            ...
            if (airport != null)
            {
                ...
                airport.SetarSemEnergia(false);
                EnsureAirportIdentity(airport);
                Debug.Log("[IA02 Military] Aeroporto militar criado no create: " + built.name + " pos=" + built.transform.position.ToString("F2"));
            }
            return airport;
        }
        ...
        private bool SpawnFighterAtMilitaryAirport(DadosConstrucao item, string orderId = "")
        {
            ...
            if (!IsUsableAircraftPrefab(fallback)
                || !IA02MilitaryCatalogPolicy.IsAllowedPrefab(fallback, controller != null ? controller.FichasMilitaresPermitidas : null))
            {
            ...
            if (aeroportoProprio != null)
            {
                ...
                if (IsUsableAircraftPrefab(aeronave)
                    && IA02MilitaryCatalogPolicy.IsAllowedPrefab(aeronave, controller != null ? controller.FichasMilitaresPermitidas : null))
                {
                    aeroportoProprio.ComprarAviaoIAImediato(aeronave, orderId);
                    issuedFighters++;
                    lastFighterOrderAt = Time.time;
                    Debug.Log("[IA02 Military] Caca estacionado no aeroporto proprio (recuperacao): "
            ...
            if (controller != null)
            {
                hasAnchor = controller.TryResolveConstructionAnchor(IA02IntentType.BuildMilitaryAirport, out anchor, out rotation);
            }
            ...
            if (!hasAnchor)
            {
                IA02BuildSlot[] slots = UnityEngine.Object.FindObjectsByType<IA02BuildSlot>(FindObjectsInactive.Include, FindObjectsSortMode.None);
            ...
            issuedFighters++;
            lastFighterOrderAt = Time.time;
            Debug.Log("[IA02 Military] Caca liberado no create do aeroporto militar: " + fallback.name);
            return true;
        }
        ...
        private bool TryProduceNaval(DadosConstrucao item, string label, string orderId = "")
        {
            ...
            for (int i = 0; i < registeredShipyardsBuffer.Count; i++)
            {
                ...
                if (BelongsToTeam(shipyard != null ? shipyard.gameObject : null) && shipyard.ConstruirUnidade(item.PrefabDaUnidade, orderId))
                {
                    ...
                    Debug.Log("[IA02 Military] Navio enfileirado no estaleiro proprio: " + shipyard.name + " -> " + item.GetDisplayName());
            ...
            if (PermiteInfraestruturaInicialAutomatica && Time.time >= nextShipyardRecoveryAt)
            {
                ...
                {
                    IA02MilitaryProductionGuard.ConfirmQueued(orderId, recovered.GetInstanceID(), Time.time);
                    Debug.Log("[IA02 Military] Estaleiro recuperado no create e navio enfileirado: " + recovered.name);
                    RegistrarCompraNaval();
                    return true;
                }
            }
        ...
        private Estaleiro EnsureOwnShipyard()
        {
            ...
            Vector3 anchor = Vector3.zero;
            Quaternion rotation = Quaternion.identity;
            bool hasAnchor = controller.TryResolveConstructionAnchor(IA02IntentType.BuildShipyard, out anchor, out rotation);
            if (!hasAnchor)
            {
                Debug.LogWarning("[IA02 Military] Create de estaleiro naval nao resolvido para a IA02.");
                return null;
            }
            if (!controller.IsPositionInsidePreparedTerritory(anchor, 220f))
            {
                Debug.LogWarning("[IA02 Military] Anchor de estaleiro fora do territorio preparado; criacao cancelada.");
                return null;
            }
            ...
            if (blueprint == null || !blueprint.TryGetPrefabBasico(out prefab) || prefab == null)
            {
                Debug.LogWarning("[IA02 Military] Ficha/prefab do estaleiro naval nao encontrado.");
                return null;
            }
            ...
            if (shipyard != null)
            {
                ...
                Debug.Log("[IA02 Military] Estaleiro naval criado no create: " + built.name + " pos=" + built.transform.position.ToString("F2"));
            }
            return shipyard;
        }
        ...
        private bool TryEmergencySpawn(DadosConstrucao item, string label, Transform anchor = null, string orderId = "")
        {
            ...
            if (identity == null) identity = unit.AddComponent<IdentidadeUnidade>();
            identity.teamID = context.TeamId;
            identity.nomeDoPais = controller != null ? controller.NationName : "IA02";
            ...
            IA02MilitaryProductionGuard.Complete(orderId, Time.time);
            CombustivelUnidade.Garantir(unit, true);
            DiagnosticoDesempenhoJogo.RegistrarProducao(item.GetDisplayName(), "IA02_ReservaMilitar");
            return true;
        }
        ...
        {
            return IA02MilitaryProductionGuard.CountOwnedUnique(context.TeamId, type);
        }

        private int CountTanks()
        {
            return IA02MilitaryProductionGuard.CountOwnedUnique(context.TeamId, TipoUnidade.Veiculo, id =>
        ...
        {
            return IA02MilitaryProductionGuard.CountOwnedUnique(context.TeamId, TipoUnidade.Aereo, id =>
        ...
        private bool BelongsToTeam(GameObject go)
        {
            ...
            if (manager != null && manager.WorldRegistry != null)
            {
                ...
                for (int i = 0; i < records.Count; i++)
                {
                    ...
                    if (record != null && record.Kind == IA02WorldEntityKind.Structure
        ...
        private bool BelongsToOwnedStructure(GameObject go)
        {
            ...
            {
                IA02WorldEntityRecord record = records[i];
                if (record == null || record.Kind != IA02WorldEntityKind.Structure) continue;
