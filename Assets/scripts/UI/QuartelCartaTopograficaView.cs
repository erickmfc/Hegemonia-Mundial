...
// Visual telemetry cache for the Quartel nautical chart.
...
/// A view e deliberadamente separada do controlador visual para que a carta
/// possa ser desenhada em UI Toolkit ou no fallback IMGUI do Game View.
/// </summary>
[DisallowMultipleComponent]
public sealed class QuartelCartaTopograficaViewSource : MonoBehaviour
{
    ...
    private UnidadeTelemetria CriarUnidade(IdentidadeUnidade identidade, Vector3 posicao, bool aliada)
    {
        ...
        if (memoria.inicializado)
        {
            memoria.distanciaPercorrida += Vector3.Distance(memoria.ultimaPosicao, posicao);
        }
        ...
        else if (aviao != null && aviao.alvoGPSVoo != Vector3.zero)
        {
            ...
            possuiDestino = aviao.estadoAtual == ControleAviao.EstadoAviao.EmMissao
        ...
        float velocidade = 0f;
        if (aviao != null) velocidade = aviao.VelocidadeVooAtual;
        else if (navio != null) velocidade = navio.VelocidadeAtual;
        else if (controle != null) velocidade = controle.ObterVelocidadeAtualReal();
        ...
        ControleSubmarino submarinoLeitura = submarino;
        if (submarinoLeitura != null)
        {
            misseisDisponiveis = submarinoLeitura.misseisDisponiveis;
            misseisMaximos = Mathf.Max(misseisDisponiveis, objeto.GetComponentsInChildren<Transform>(true).Length > 0 ? 22 : misseisDisponiveis);
            torpedosDisponiveis = submarinoLeitura.torpedosDisponiveis;
        ...
        string estado = aviao != null ? aviao.estadoAtual.ToString()
            : submarino != null ? (submarino.estaSubmerso ? "SUBMERSO" : "SUPERFICIE")
            : controle != null ? controle.OrdemAtual.ToString() : "SEM CONTROLADOR";
        string tipo = identidade.tipoUnidade.ToString().ToUpperInvariant();
        string missao = e3 != null ? "Reconhecimento e retransmissao"
            : controle != null && controle.OrdemAtual == OrdemControleUnidade.Patrulhando ? "Patrulha"
            : estado;
