...
namespace Hegemonia.AI.IA01
{
    ...
    /// </summary>
    public sealed class IA01WarAdvanceZone : MonoBehaviour, IWarAdvanceZone
    {
            ...
            float damage, string infrastructureHit, bool targetDestroyed, bool attackerStillPresent, bool confirmed, float now)
        {
            ...
            for (int i = 0; i < incidentes.Count; i++)
            {
                if (incidentes[i].AttackerTeamId == attackerTeamId
                    && now - incidentes[i].LastDetectionTime <= Mathf.Max(1f, janelaAgregacaoSegundos))
                {
                    existente = incidentes[i];
                    break;
                }
            }
            if (existente == null)
        ...
        {
            if (incidente.TargetDestroyed || incidente.DamageCaused >= 100f)
                return WarAdvanceIncidentSeverity.Critica;
            if (incidente.AttackerStillPresent && incidente.AttackCount >= 2 || incidente.DamageCaused >= 35f || incidente.MissilesDetected >= 3)
                return WarAdvanceIncidentSeverity.Alta;
            if (incidente.MissilesDetected > 0 || incidente.DamageCaused > 0f || incidente.ShotsDetected >= 3 || incidente.AttackCount >= 2)
                return WarAdvanceIncidentSeverity.Media;
